...
- Current-user startup support through the Windows Startup folder shortcut, with registry fallback.
- Startup launches with a hidden `--startup` mode so login does not open the main app window or a console.
- Best-effort auto restart on Rust panic.

## Install And Run

1. Download `SmartClipboard-v0.1.0-windows-x64.zip` from GitHub Releases.
...
git commit -m "Release Smart Clipboard v0.1.0"
git branch -M main
...
5. Upload `SmartClipboard-v0.1.0-windows-x64.zip`.
